...
std::bitset<160 * 144 * 2> gbee::Emulator::GetFrameBuffer( const uint8_t instanceID ) const				{ return Instances[instanceID].GetFramebuffer(); }
void gbee::Emulator::SetKeyState( const uint8_t key, const bool state, const uint8_t instanceID ) const { Instances[instanceID].SetKey( (Key)key, state ); }
void gbee::Emulator::SetPauseState( const bool state, const uint8_t instanceID ) const					{ Instances[instanceID].SetPaused( state ); }
bool gbee::Emulator::GetPauseState( const uint8_t instanceID ) const									{ return Instances[instanceID].GetPaused(); }
